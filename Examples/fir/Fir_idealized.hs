...
data PushArray a = forall b c . (S.Elt b, S.Elt c) =>
                   PushArray ((Exp Int -> Exp a -> Filter b c ()) -> Filter b c ())
type PullArray a = (Exp Int -> Exp a)

-- | Turn a manifest array bound to a variable into an array that can be used with
-- the combinators.
namedArr :: Var (SArray a) -> SArray a
namedArr vr =
  SArray
...
take :: Elt a => Exp Int -> Stream a -> SArray a
...
-- with a for loop.
loopPusher :: Elt a => Exp Int -> (Var Int -> Exp a) -> PushArray a 
loopPusher len body = 
  PushArray $ \ rcvr -> do
